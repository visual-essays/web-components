import { c as createCommonjsModule, a as commonjsGlobal, o as openseadragon } from './openseadragon.js';
...
async function getManifest(manifestId, refresh = false) {
...
function findItem(toMatch, current, seq = 1) {
  const found = _findItems(toMatch, current);
  return found.length >= seq ? found[seq - 1] : null;
}
function _findItems(toMatch, current, found = []) {
  found = found || [];
  if (current.items) {
    for (let i = 0; i < current.items.length; i++) {
      ...
      if (isMatch)
        ...
        _findItems(toMatch, item, found);
    }
  }
  return found;
}
function imageInfo(manifest, seq = 1) {
  return findItem({ type: 'Annotation', motivation: 'painting' }, manifest, seq).body;
}
async function loadManifests(manifestUrls, refresh = false) {
  let requests = manifestUrls
    .map(manifestId => manifestId.indexOf('http') === 0
    ? manifestId
    : `${iiifServer}/${manifestId}/manifest.json`)
    .map(manifestUrl => {
    ...
    return fetch(manifestUrl, ['localhost', 'iiif.visual-essays.net'].includes(new URL(manifestUrl).hostname)
...
function parseImageOptions(str) {
  let elems = (str === null || str === void 0 ? void 0 : str.split('/')) || [];
  // let seq = 1
  let region = 'full';
  let size = 'full';
  let rotation = '0';
...
function parseRegionString(region, viewer) {
  ...
  if (ints.length === 4) {
    if (s1.length === 1 || (s1.length === 2 && (s1[0] === 'px' || s1[0] === 'pixel'))) {
      ...
      if (size.x > 0 && size.y > 0) {
        return viewer.viewport.imageToViewportRectangle(Math.round(size.x * ints[0] / 100), Math.round(size.y * ints[1] / 100), Math.round(size.x * ints[2] / 100), Math.round(size.y * ints[3] / 100));
      }
    }
  }
}
    ...
    if (resp.ok) {
      ...
      sparqlResp.results.bindings.forEach((rec) => {
        ...
        if (!entityData[qid]) {
          ...
          if (rec.description)
            entityData[qid].description = rec.description.value;
          if (rec.alias)
            entityData[qid].aliases = [rec.alias.value];
          if (rec.coords)
            entityData[qid].coords = rec.coords.value.slice(6, -1).split(' ').reverse().join(',');
          if (rec.wikipedia)
            entityData[qid].wikipedia = rec.wikipedia.value;
          if (rec.image) {
          ...
          if (rec.alias)
            entityData[qid].aliases.push(rec.alias.value);
        }
      });
      return entityData;
    }
...
async function getDepictedEntities(hash) {
  let depicted = [];
  let payload = { query: { query_string: { query: `_id:${hash}` } }, size: 1 };
  // console.log(`getDepictedEntities: hash=${hash}`, payload)
  let resp = await fetch('https://www.jstor.org/api/labs-search-service/labs/about/', {
    method: 'POST',
...
export { getDepictedEntities as a, getEntityData as b, summary as c, source as d, sourceId as e, imageDataUrl as f, getManifest as g, parseRegionString as h, imageInfo as i, isNum as j, loadManifests as k, label as l, parseImageOptions as p, sha256 as s, thumbnail as t };
