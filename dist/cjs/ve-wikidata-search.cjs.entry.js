...
const WikidataSearch = class {
  constructor(hostRef) {
    ...
    this.entitySelected = index.createEvent(this, "entitySelected", 7);
    this.language = 'en';
    this.wdResults = [];
  ...
  render() {
    return [
      index.h("div", { class: "wikidata-search" }, index.h("div", { id: "autocompleteContainer", class: "autocomplete__container", role: "combobox" }, index.h("input", { id: "autocompleteInput", class: "autocomplete__input", onKeyUp: this.inputHandler.bind(this), autocomplete: "off", role: "textbox", placeholder: "Search for entity" }), index.h("button", { id: "autocompleteDropdownArrow", class: "autocomplete__dropdown-arrow", style: { visibility: this.wdResults.length > 0 ? 'visible' : 'hidden' }, onClick: this.toggleDropdown.bind(this), "aria-label": "toggle dropdown" }, index.h("svg", { "fill-rule": "evenodd", height: "5", viewBox: "0 0 10 5", width: "10" }, index.h("title", null, "Open drop down"), index.h("path", { d: "M10 0L5 5 0 0z" }))), index.h("ul", { id: "autocompleteResults", class: "autocomplete__results", role: "listbox" }, this.wdResults.map((item) => index.h("li", { onClick: this.itemSelected.bind(this, item) }, index.h("ul", null, index.h("li", null, index.h("span", { class: "label", innerHTML: item.label }), item.aliases ? index.h("span", { class: "aliases" }, "(", item.aliases.join(', '), ")") : ''), index.h("li", { class: "description", innerHTML: item.description })))), this.searchContinue
        ? index.h("li", { onClick: this.doSearch.bind(this), class: "continue", "v-if": "searchContinue" }, "More...")
        : '')))
    ];
  }
...
exports.ve_wikidata_search = WikidataSearch;
