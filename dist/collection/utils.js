...
export async function getManifest(manifestId, refresh = false) {
...
export async function prezi2to3(manifest) {
...
export function findItem(toMatch, current, seq = 1) {
...
export function imageInfo(manifest, seq = 1) {
  return findItem({ type: 'Annotation', motivation: 'painting' }, manifest, seq).body;
}
export async function loadManifests(manifestUrls, refresh = false) {
...
export async function imgUrlFromManifest(manifestUrl, forceImage = false) {
...
export function parseImageOptions(str) {
...
export function parseRegionString(region, viewer) {
...
let entityData = {};
export async function getEntityData(qids = [], language = 'en') {
...
export async function getDepictedEntities(hash) {
  let depicted = [];
...
export function label(manifest, language = 'en') {
  return manifest ? _value(manifest.label, language) : null;
}
export function summary(manifest, language = 'en') {
  return manifest ? _value(manifest.summary, language) : null;
}
export function source(manifest) {
  if (!manifest)
    return null;
...
export function sourceId(manifest) {
  if (!manifest)
    return null;
  let parsed = new URL(manifest.id);
  if (parsed.origin === iiifServer) {
...
export function thumbnail(manifest) {
  if (!manifest)
    return null;
